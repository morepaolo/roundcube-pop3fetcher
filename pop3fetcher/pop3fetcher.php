...
class pop3fetcher extends rcube_plugin
{
	...
	private function _decrypt_pass($pass) {
		...
		$e = new \pop3fetcher\Encryption($this->crypt_key, $this->iv_key);
		return $e->decrypt($p);
	}
	...
	function init(){
		...
		if(!file_exists('plugins/pop3fetcher/skins/' . $skin . '/pop3fetcher.css')){
			if(!file_exists('plugins/pop3fetcher/skins/larry/pop3fetcher.css'))
				$this->skin = "larry";
			else
				$this->skin = "larry";
		}
		...
		$this->load_config();
		$this->crypt_key = $this->rcmail->config->get("pop3fetcher_crypt_key", "%E`c{2;<J2F^4_&._BxfQ<5Pf3qv!m{e");
		...
		if(
			...
			foreach($accounts as $key => $val){
				...
				$c = POP35::connect($val['pop3fetcher_serveraddress'], $val['pop3fetcher_username'], $this->_decrypt_pass($val['pop3fetcher_password']), intval($val['pop3fetcher_serverport']), $val['pop3fetcher_ssl'], 100);
				//print_r($_RESULT);
				if($c){
	...
	public function render_page($params){
		$this->include_stylesheet('skins/larry/pop3fetcher.css');
		return($params);
	}
	...
	function settings($args){
		$framed = rcube_utils::get_input_value('_framed', rcube_utils::INPUT_POST);
		if ($args['section'] == 'pop3fetcher') {

			if($this->rcmail->config->get("automatically_check_for_updates", yes)){
			...
			if(count($accounts) > 0){
				...
				foreach($accounts as $key => $val){
					...
					$args['blocks']['main']['options']['pop3fetcher_account_'.$val['pop3fetcher_id']]['title'] = html::quote($val['pop3fetcher_email']);
	...
	function edit_do(){
		...
		$pop3fetcher_id = rcube_utils::get_input_value('_pop3fetcher_id', rcube_utils::INPUT_POST);
		$pop3fetcher_email = rcube_utils::get_input_value('_pop3fetcher_email', rcube_utils::INPUT_POST);
		$pop3fetcher_username = rcube_utils::get_input_value('_pop3fetcher_username', rcube_utils::INPUT_POST);
		$pop3fetcher_password = rcube_utils::get_input_value('_pop3fetcher_password', rcube_utils::INPUT_POST);
		$pop3fetcher_serveraddress = rcube_utils::get_input_value('_pop3fetcher_serveraddress', rcube_utils::INPUT_POST);
		$pop3fetcher_serverport = rcube_utils::get_input_value('_pop3fetcher_serverport', rcube_utils::INPUT_POST);
		$pop3fetcher_ssl = rcube_utils::get_input_value('_pop3fetcher_ssl', rcube_utils::INPUT_POST);
		$pop3fetcher_leaveacopy = rcube_utils::get_input_value('_pop3fetcher_leaveacopy', rcube_utils::INPUT_POST);
		$pop3fetcher_provider = rcube_utils::get_input_value('_pop3fetcher_provider', rcube_utils::INPUT_POST);
		$pop3fetcher_testconnection = rcube_utils::get_input_value('_pop3fetcher_testconnection', rcube_utils::INPUT_POST);
		$pop3fetcher_defaultfolder = rcube_utils::get_input_value('_pop3fetcher_defaultfolder', rcube_utils::INPUT_POST);
		...
		if($pop3fetcher_testconnection=="true"&&!$this->test_connection($pop3fetcher_username,$pop3fetcher_password,$pop3fetcher_serveraddress,$pop3fetcher_serverport,$pop3fetcher_ssl)){
			...
			$ret = $rcmail->db->query($query, $pop3fetcher_email, $pop3fetcher_username, $this->_encrypt_pass($pop3fetcher_password), $pop3fetcher_serveraddress, $pop3fetcher_serverport, $pop3fetcher_ssl, $pop3fetcher_leaveacopy, $pop3fetcher_provider, $pop3fetcher_defaultfolder, $pop3fetcher_id);
			if($ret){
	...
	function accounts_edit($args){
		...
		$pop3fetcher_id = rcube_utils::get_input_value('_pop3fetcher_id', rcube_utils::INPUT_GET);
		...
		$out .= $this->accounts_form_content($arr['pop3fetcher_email'], $arr['pop3fetcher_username'], $this->_decrypt_pass($arr['pop3fetcher_password']), $arr['pop3fetcher_serveraddress'], $arr['pop3fetcher_serverport'], $arr['pop3fetcher_ssl'], $arr['pop3fetcher_leaveacopyonserver'], $arr['pop3fetcher_provider'], $arr['default_folder'], false);
	...
	function accounts_form_content($email="",$username="",$password="",$server="", $port="", $useSSL='none', $leave_a_copy=true, $provider="", $default_folder="", $show_import_old_messages_option=true){ 
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_email')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_username')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_password')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_provider')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_serveraddress')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td>%s</td><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td>%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_serverport')),
						$input_pop3fetcher_serverport->show($port),
						$field_id2,
						html::quote($this->gettext('account_usessl')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" colspan=\"3\" class=\"title\"><label for=\"%s\">%s</label>:</td><td>%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_leaveacopy')),
		...
		foreach ($a_folders as $ifolder){
			if($ifolder==$custom_folder_name)
				$found=true;
			$this->rcmail->build_folder_tree($a_mailboxes, $ifolder, $delimiter);
		}
		$input_folderlist = new html_select(array('name' => '_pop3fetcher_defaultfolder', 'id' => $field_id));
		$this->rcmail->render_folder_tree_select($a_mailboxes, $field_id, 100, $input_folderlist, false);
		...
		$out .= sprintf("<tr><td valign=\"middle\" class=\"title\"><label for=\"%s\">%s</label>:</td><td colspan=\"3\">%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('account_default_folder')),
		...
		$out .= sprintf("<tr><td valign=\"middle\" colspan=\"3\" class=\"title\"><label for=\"%s\">%s</label>:</td><td>%s</td></tr>\n",
						$field_id,
						html::quote($this->gettext('test_connection_on_save')),
		...
		if($show_import_old_messages_option){
			...
			$out .= sprintf("<tr><td valign=\"middle\" colspan=\"3\" class=\"title\"><label for=\"%s\">%s</label>:</td><td>%s</td></tr>\n",
							$field_id,
							html::quote($this->gettext('account_import_old_messages')),
	...
	*/
	...
	function add_do(){
		...
		$pop3fetcher_testconnection = rcube_utils::get_input_value('_pop3fetcher_testconnection', rcube_utils::INPUT_POST);
		$pop3fetcher_defaultfolder = rcube_utils::get_input_value('_pop3fetcher_defaultfolder', rcube_utils::INPUT_POST);
		$pop3fetcher_import_old_messages = rcube_utils::get_input_value('_pop3fetcher_import_old_messages', rcube_utils::INPUT_POST);
		...
		if($pop3fetcher_testconnection=="true"&&!$this->test_connection($pop3fetcher_username,$pop3fetcher_password,$pop3fetcher_serveraddress,$pop3fetcher_serverport,$pop3fetcher_ssl)){
			...
			if(!is_array($arr)){
				...
				$ret = $rcmail->db->query($query,
										  ...
										  $this->_encrypt_pass($pop3fetcher_password),
